...
void sportdh(uint16_t sp,uint16_t Ens,int16_t  pei,uint16_t yit)//sp为速度,Ens为编码值，pei为巡线选项,设置为1则是黄泽鸣的,设置为2则是敖思宇的
{
      ...
      if(last != state)
		  ...
		  if(abs(state) > 5)
		  {
			  mydriver((int16_t)(0.1f*buf), (int16_t)(0.1f*buf*state));  //10,15, 0.06
				//mydriver(abs((int16_t )(is*0.3f)),50*state-0.47f*is);
...
void program3(void)   
{
	...
	int16_t pei1[20][2]={{2,2},{4,1},{6,1},{7,1},{8,1}};//巡线模式(默认为0)
	int16_t yit1[20][2]={{5,36}};//转弯光电倍率(默认为50)
	int16_t pei[20];//巡线模式
	int16_t yit[20];//转弯光电倍率(已弃用)
	for(uint16_t i = 0; i < 20; i++)
	{
				pei[i]=0;
				yit[i]=50;
